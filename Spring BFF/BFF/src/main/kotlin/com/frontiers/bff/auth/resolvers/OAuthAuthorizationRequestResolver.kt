package com.frontiers.bff.auth.resolvers

import com.frontiers.bff.auth.resolvers.customizers.AdditionalParamsAuthorizationRequestCustomizer
import com.frontiers.bff.auth.resolvers.customizers.CompositeOAuth2AuthorizationRequestCustomizer
...
// adapted from:
// https://github.com/ch4mpy/spring-addons/blob/master/spring-addons-starter-oidc/src/main/java/com/c4_soft/springaddons/security/oidc/starter/reactive/client/SpringAddonsServerOAuth2AuthorizationRequestResolver.java

/**
 * Adds the success uri, and failure uri to the user's session (for post-login redirections)
 * Potentially modifies the re-direct URI, to handle a proxy server configuration
 * Also adds PKCE, and the ability to add extra authorization parameters, if needed
 */
@Component
internal class OAuthAuthorizationRequestResolver(
    ...
    private fun savePostLoginUrisInSession(exchange: ServerWebExchange): Mono<WebSession> {
        ...
        return exchange.session.map { session ->
            ...
            logger.info("SAVING POST LOGIN URIS TO SESSION ID: ${session.id}")
    ...
    private fun postProcess(request: OAuth2AuthorizationRequest): OAuth2AuthorizationRequest {
        ...
        // log the changes
        logger.info("Changed OAuth2AuthorizationRequest redirectUri from {} to {}", original, redirectUri)
